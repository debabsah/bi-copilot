...
description: Use when a BI/analytics professional starts an OPEN-ENDED look at data — "find insights", "explore this dataset", "what drives [metric]", "any patterns here", "dig into the segments" — and the findings must survive scrutiny later. Harnesses the exploration itself: pre-registers the questions and the finding-bar BEFORE results are examined, logs every cut (N cuts ⇒ ~N/20 false hits expected at α≈.05, stated next to any striking pattern), keeps effect sizes and bases ahead of significance talk, labels everything Exploratory-found until a pre-specified hold-out check YOU run confirms it, and records dead ends. Read-only: writes the exact cuts and checks for you to run and paste back; never runs the analysis. A number already wrong or moved is triage-my-number; a causal/A-B claim is audit-my-experiment; source premises are audit-my-assumptions; communicating confirmed findings is brief-my-findings.
