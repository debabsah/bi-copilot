...
4. **Grade each finding** — **Blocking** (ships a wrong number) / **Latent** (correct today, will break: edge / SCD / late data) / **Advisory** (correct but unclear). Each finding: location · failure mode · what wrong result it produces · fix direction.
